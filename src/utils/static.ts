import config from './config';

export const metaData = {
  image: `${config.SITE_URL}/media/landing.png`,
  keywords: 'Waste disposal, Repurposing, Solid waste management, Waste collection, Environmental waste management, Trash disposal, Waste reduction, Waste management services, Sustainable waste management, Residential waste management, Green waste management, Municipal waste management, Waste management company, Waste management solutions, Waste management and repurposing, Waste management best practices, Waste management guidelines, Alterbin, Optimal',
  url: config.SITE_URL,
  geo: {
    placeName: 'Ibadan, Oyo, Nigeria',
    position: '6.5474147;3.3639306',
    region: 'NG',
  },
  siteName: 'Alterbin | Optimal',
  title: 'Alterbin | Optimal',
  description: 'Give Old Items a New Story.',
};
